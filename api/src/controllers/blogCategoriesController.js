import { sendApiError } from "../utils/apiError.js";
// api/src/controllers/blogCategoriesController.js
import { query } from "../config/database.js";

const normalizeSlug = (s) =>
  String(s ?? "")
    .trim()
    .toLowerCase()
    .replace(/['"]/g, "")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");

async function categoryExistsBySlug(slug, excludeId = null) {
  if (!excludeId) {
    const rows = await query(
      "SELECT id FROM blog_categories WHERE slug = ? LIMIT 1",
      [slug]
    );
    return Boolean(rows[0]);
  }
  const rows = await query(
    "SELECT id FROM blog_categories WHERE slug = ? AND id <> ? LIMIT 1",
    [slug, excludeId]
  );
  return Boolean(rows[0]);
}

export const getBlogCategories = async (req, res) => {
  try {
    const categories = await query(
      "SELECT * FROM blog_categories ORDER BY display_order, name"
    );
    res.json({ categories });
  } catch (error) {
    console.error("Get blog categories error:", error);
    sendApiError(res, error);
  }
};

export const createBlogCategory = async (req, res) => {
  try {
    const {
      name,
      slug,
      description = null,
      image_url = null,
      display_order = 0,
      parent_id = null,
    } = req.body ?? {};

    if (!name) return res.status(400).json({ error: "name is required" });

    const finalSlug = normalizeSlug(slug || name);
    if (!finalSlug) return res.status(400).json({ error: "Invalid slug" });

    if (await categoryExistsBySlug(finalSlug)) {
      return res.status(409).json({ error: "Slug already exists" });
    }

    if (parent_id) {
      const parent = await query(
        "SELECT id FROM blog_categories WHERE id = ? LIMIT 1",
        [parent_id]
      );
      if (!parent[0]) return res.status(400).json({ error: "Invalid parent_id" });
    }

    const idRows = await query("SELECT UUID() AS id");
    const id = idRows[0].id;

    await query(
      `
      INSERT INTO blog_categories (id, parent_id, name, slug, description, image_url, display_order)
      VALUES (?, ?, ?, ?, ?, ?, ?)
      `,
      [id, parent_id, name, finalSlug, description, image_url, Number(display_order) || 0]
    );

    const [category] = await query("SELECT * FROM blog_categories WHERE id = ?", [
      id,
    ]);

    res.status(201).json({ category });
  } catch (error) {
    console.error("Create blog category error:", error);
    sendApiError(res, error);
  }
};

export const updateBlogCategory = async (req, res) => {
  try {
    const { id } = req.params;

    const existing = await query(
      "SELECT * FROM blog_categories WHERE id = ? LIMIT 1",
      [id]
    );
    if (!existing[0]) return res.status(404).json({ error: "Category not found" });

    const { name, slug, description, image_url, display_order, parent_id } =
      req.body ?? {};

    const patch = [];
    const params = [];

    if (name !== undefined) {
      if (!name) return res.status(400).json({ error: "name cannot be empty" });
      patch.push("name = ?");
      params.push(name);
    }

    if (slug !== undefined) {
      const finalSlug = normalizeSlug(slug || "");
      if (!finalSlug) return res.status(400).json({ error: "Invalid slug" });

      if (await categoryExistsBySlug(finalSlug, id)) {
        return res.status(409).json({ error: "Slug already exists" });
      }

      patch.push("slug = ?");
      params.push(finalSlug);
    }

    if (description !== undefined) {
      patch.push("description = ?");
      params.push(description);
    }

    if (image_url !== undefined) {
      patch.push("image_url = ?");
      params.push(image_url);
    }

    if (display_order !== undefined) {
      patch.push("display_order = ?");
      params.push(Number(display_order) || 0);
    }

    if (parent_id !== undefined) {
      if (parent_id === id) {
        return res.status(400).json({ error: "parent_id cannot be same as id" });
      }
      if (parent_id) {
        const parent = await query(
          "SELECT id FROM blog_categories WHERE id = ? LIMIT 1",
          [parent_id]
        );
        if (!parent[0])
          return res.status(400).json({ error: "Invalid parent_id" });
      }
      patch.push("parent_id = ?");
      params.push(parent_id || null);
    }

    if (patch.length) {
      await query(`UPDATE blog_categories SET ${patch.join(", ")} WHERE id = ?`, [
        ...params,
        id,
      ]);
    }

    const [category] = await query("SELECT * FROM blog_categories WHERE id = ?", [
      id,
    ]);
    res.json({ category });
  } catch (error) {
    console.error("Update blog category error:", error);
    sendApiError(res, error);
  }
};

export const deleteBlogCategory = async (req, res) => {
  try {
    const { id } = req.params;

    const existing = await query(
      "SELECT id FROM blog_categories WHERE id = ? LIMIT 1",
      [id]
    );
    if (!existing[0]) return res.status(404).json({ error: "Category not found" });

    // If posts are linked, forbid delete to avoid accidental data loss
    const linked = await query(
      "SELECT 1 FROM blog_post_category_pivot WHERE category_id = ? LIMIT 1",
      [id]
    );
    if (linked.length) {
      return res.status(409).json({
        error: "Category is used by blog posts. Remove links before deleting.",
      });
    }

    await query("DELETE FROM blog_categories WHERE id = ?", [id]);

    res.json({ success: true });
  } catch (error) {
    console.error("Delete blog category error:", error);
    sendApiError(res, error);
  }
};
